...
import { Coins, Gift, Zap, ChevronLeft, ChevronRight } from 'lucide-react';
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-6">
                ...
                {airdrops.map((_, index) => (
                  <button
                    ...
                    className={`w-2 h-2 rounded-full transition-all duration-300 ${
                      index === currentIndex ? 'bg-[#EE5946] w-8' : 'bg-white/30'
                    }`}
              ...
              <div className="flex space-x-2">
                <button
                  onClick={prevSlide}
                  className="p-2 bg-white/10 hover:bg-white/20 border border-white/20 rounded-full transition-all duration-300"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {airdrops.map((airdrop, index) => (
                <div
                  key={airdrop.id}
                  className={`group relative bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 transition-all duration-700 cursor-pointer ${
                    index === currentIndex 
                      ? 'bg-white/10 border-[#EE5946]/50 shadow-[0_0_30px_rgba(238,89,70,0.2)] scale-105' 
                      : 'hover:bg-white/8 hover:border-blue-400/30'
                  } ${hoveredAirdrop === airdrop.id ? 'transform ' : ''}`}
                ...
                >
                  ...
                  <div className="absolute top-2 right-2">
                    <div className="relative">
                      <Coins className={`w-6 h-6 text-yellow-400 transition-transform duration-500 ${
                        hoveredAirdrop === airdrop.id ? 'animate-bounce' : ''
                      }`} />
